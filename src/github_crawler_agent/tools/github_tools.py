...
def get_github_repos(username: str) -> str:
    """
    Fetches the top 10 public repositories of a GitHub user, sorted by stars.

    Args:
        username: The GitHub username of the candidate.

    Returns:
        A formatted string listing the top repositories with details.
    """
    try:
        ...
        result = f"Top Repositories for {username}:\n"
        result += "─" * 50 + "\n"
...
def get_candidate_github_report(username: str) -> str:
    ...
    profile, top repos, languages per top repo, and README highlights for top 3 repos.
    """
    try:
        sections = []
        ...
        # Fetch repos once as structured data so we can reliably pick top repos.
        ...
        if language_blocks:
            sections.append("Language Breakdown Across Top Repos:\n" + "\n".join(language_blocks))
        ...
        if readme_blocks:
            sections.append("README Highlights (Top 3 Repos):\n" + "\n\n".join(readme_blocks))
